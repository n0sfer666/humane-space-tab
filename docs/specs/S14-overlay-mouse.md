...
A slot is a place in the ribbon, not an application: from five applications on the ribbon
is a carousel (S07), so the same slot carries a different entry after every step. So the hit test answers a slot, and the
...
- [x] A click on a turning ribbon commits the entry the slot carries now, not the one it carried before the step.
