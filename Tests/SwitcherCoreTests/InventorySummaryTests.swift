...
struct InventorySummaryTests {
    ...
    private func body(for applications: [SwitchableApplication]) -> String {
        let text = InventorySummary.text(for: applications, layer: .onScreen)
        return text.split(separator: "\n", maxSplits: 1).dropFirst().joined()
    }

    @Test("the layer summary names the layer above the applications")
    func layerHeader() {
        let text = InventorySummary.text(for: [application("Notes")], layer: .onScreen)
        #expect(text == "Space layer: public (on-screen windows)\nNotes — no windows")
    }

    @Test("the layer summary keeps the empty notice below the header")
    func layerHeaderWithEmptyInventory() {
        #expect(
            InventorySummary.text(for: [], layer: .skyLight)
                == "Space layer: private (SkyLight)\nNo switchable applications."
        )
    }

    @Test("each application is one line with its window counts by visibility")
    func lineFormat() {
        let text = body(for: [application("Notes", windows: [.onScreen, .minimised])])
    ...
    func omitsEmptyGroups() {
        let text = body(for: [application("Notes", windows: [.onScreen])])
